  ...
  filters.forEach((filter) => {
    ...
    switch (filter.operator) {
      ...
      case 'eq':
        ...
        } else if (typeof filter.value === 'string' && filter.value === 'false') {
      ...
      case 'lt':
        if (filter.variant === 'number' || filter.variant === 'range') {
          conditions.push({ [fieldName]: { lt: filter.value } });
      ...
      case 'lte':
        if (filter.variant === 'number' || filter.variant === 'range') {
          conditions.push({ [fieldName]: { lte: filter.value } });
        } else if (filter.variant === 'date' && typeof filter.value === 'string') {
          const date = new Date(Number(filter.value));
          date.setHours(23, 59, 59, 999);
      ...
      case 'gt':
        if (filter.variant === 'number' || filter.variant === 'range') {
          conditions.push({ [fieldName]: { gt: filter.value } });
        } else if (filter.variant === 'date' && typeof filter.value === 'string') {
          const date = new Date(Number(filter.value));
      ...
      case 'gte':
        if (filter.variant === 'number' || filter.variant === 'range') {
          conditions.push({ [fieldName]: { gte: filter.value } });
        } else if (filter.variant === 'date' && typeof filter.value === 'string') {
          const date = new Date(Number(filter.value));
          date.setHours(0, 0, 0, 0);
